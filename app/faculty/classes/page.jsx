    ...
    const fetchClassStudents = useCallback(async (cls) => {
        ...
        try {
            ...
            if (studs.length > 0) {
                ...
                if (marks?.length) {
                    setAllMarks(marks);
                    const sems = [...new Set(marks.map(m => m.semester))].sort((a, b) => a - b);
                    setAvailableSems(sems);
                    const last = sems[sems.length - 1];
                    setSelectedSem(last);
                    computeToppers(marks, studs, last);
                }
            }
        } finally { setLoadingStudents(false); }
    }, []);

    const computeToppers = (marks, studs, sem) => {
        ...
        const result = Object.entries(bySubj).map(([code, rows]) => ({
            ...
            top3: rows.sort((a, b) => b.total - a.total).slice(0, 3).map(r => ({ usn: r.usn, name: nameMap[r.usn] || r.usn, total: r.total }))
            ...
            {top10.length > 1 && (
                <div style={{ ...S.card, padding: '16px 20px', marginBottom: '24px', overflowX: 'auto' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'nowrap' }}>
                        {top10.map((s, i) => (
                            <button key={s.usn} onClick={() => { setSemFilter('all'); openStudentDrawer(s); }} style={{ background: 'var(--surface-low)', border: '1px solid var(--border)', borderRadius: '10px', padding: '8px 14px', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: '2px' }}>
            ...
            {subjectToppers.length > 0 && (
                <div style={{ ...S.card, marginBottom: '24px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
                        <div style={{ fontSize: '13px', fontWeight: 800, color: 'var(--tx-main)' }}>📚 Subject Toppers</div>
                        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                            {availableSems.map(s => (
                                <button key={s} onClick={() => { setSelectedSem(s); computeToppers(allMarks, students, s); }} style={{ padding: '4px 12px', borderRadius: '8px', fontWeight: 800, fontSize: '11px', cursor: 'pointer', border: 'none', fontFamily: 'inherit', background: selectedSem === s ? 'var(--primary)' : 'var(--surface-low)', color: selectedSem === s ? 'var(--bg)' : 'var(--tx-dim)' }}>Sem {s}</button>
                            ))}
                        </div>
                    </div>
