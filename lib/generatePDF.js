...
import { getGradePoint } from './vtuGrades';
...
function calcSGPA(marks) {
    ...
    marks.forEach(m => {
        ...
        const computed_pts = getGradePoint(grade, '2022', m.total_marks || m.total);
...
export function generateResultPDF({
    ...
    // ===== SEMESTER TABLES =====
    const semesters = Object.keys(semesterMarks).sort((a, b) => a - b);
    const sgpas = [];
    ...
    semesters.forEach((sem, idx) => {
        ...
        const { sgpa, credits: totalCredits } = calcSGPA(marks);
        sgpas.push({ semester: sem, sgpa, credits: totalCredits });
        ...
        doc.setFontSize(8);
        doc.setTextColor(120, 113, 108);
        doc.text(`SGPA: ${sgpa.toFixed(2)} · Credits: ${totalCredits}`, pageWidth - 20, y, { align: 'right' });
        ...
        autoTable(doc, {
            ...
            didParseCell: (data) => {
                if (data.column.index === 6 && data.section === 'body') {
                    if (data.cell.text[0] === 'F' || data.cell.text[0] === 'Ab') {
                        data.cell.styles.textColor = [220, 38, 38];
                    } else if (data.cell.text[0] === 'O' || data.cell.text[0] === 'S') {
    ...
    if (sgpas.length > 0) {
        ...
        autoTable(doc, {
            startY: y,
            head: [['Semester', 'SGPA', 'Credits', 'Grade Points']],
            body: sgpas.map(s => [
                `Semester ${s.semester}`,
                s.sgpa.toFixed(2),
                s.credits,
